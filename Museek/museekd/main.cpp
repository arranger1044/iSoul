...
/* NSTask does not allow processes to join a parent group
   so this function is called in a separate thread
   waiting for the parent process to die, and when it does
   museekd is terminated aswell
*/
void* WatchForParentTermination (void* arg) {	
	...
	std::cout << "Parent Process Terminated - closing museekd" << std::endl;
	museekd->reactor()->stop();
	return 0;
}
...
int main(int argc, char ** argv)
{
  ...
  /* Spawn a thread to check the parent is still alive */
  pthread_t thread;
